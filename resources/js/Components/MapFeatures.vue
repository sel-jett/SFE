...
<template>
  <Listbox as="div" class="w-[170px] absolute lg:left-80 xs:left-12 xs:top-12.8" v-model="selected">
    <div class="relative mt-2">
      <ListboxButton class="cursor-pointer relative w-full rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:bg-[#03d3b9] sm:text-sm sm:leading-6">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <ListboxOptions class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          <ListboxOption as="template" v-for="city in cities" :key="city.id" :value="city" v-slot="{ active, selected }">
            <li :class="[active ? 'bg-[#03d3b9] text-white' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-3 pr-9']">
              <div @click="selectedResult(city)"  class="flex items-center">
                <span @click="handleCityClick(city)" :class="[selected ? 'font-semibold' : 'font-normal', 'ml-3 block truncate']">{{ city.name }}</span>
              </div>

              <span v-if="selected" :class="[active ? 'text-white' : 'text-[#03d3b9]', 'absolute inset-y-0 right-0 flex items-center pr-4']">
...
const handleCityClick = async(city) => {
  ...
  console.log('data of location : ',searchData.value.geometry);
